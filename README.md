...
```text
SmartPortAgent/
├── .venv/                                    # [自动生成] uv 创建的虚拟环境
├── data/                                     # 数据层
│   ├── vector_store_index/                   # 离线构建的向量索引 (FAISS)
│   │   ├── index.faiss                       # 向量数据文件
│   │   └── index.pkl                         # 索引元数据文件
│   ├── knowledge_base.txt                    # RAG 知识库源文件 (口岸政策/SOP)
│   └── mock_api_data.json                    # 模拟 API 的后端业务数据
├── model/                                    # 本地模型目录 (建议加入 .gitignore)
│   └── m3e-base/                             # 下载好的 Embedding 模型
│       ├── config.json
│       ├── model.safetensors
│       └── ...
├── script/                                   # 运维与辅助脚本
│   ├── build_vector_store.py                 # 离线构建向量库脚本 (一次运行，永久加速)
│   └── download_sentence_embedding_model.py  # 模型下载脚本
├── src/                                      # 核心源代码
│   ├── agent/                                # Agent 逻辑
│   │   ├── __init__.py
│   │   └── agent_creator.py                  # Agent 工厂、Prompt 模板定义
│   ├── config/                               # 配置模块
│   │   ├── __init__.py
│   │   └── settings.py                       # 全局配置 (路径、API Key、模型参数)
│   ├── rag/                                  # RAG 模块
│   │   ├── __init__.py
│   │   └── retriever_factory.py              # 检索器工厂 (已升级支持本地加载)
│   ├── tools/                                # 工具模块
│   │   ├── __init__.py
│   │   └── port_tools.py                     # 模拟 API 查询工具函数
│   └── web/                                  # [Web] 前端界面模块
│       ├── __init__.py
│       ├── app.py                            # Streamlit 主程序入口
│       ├── sidebar.py                        # 侧边栏 UI 组件
│       └── utils.py                          # CSS 样式注入与打字机效果工具
├── .env                                      # 环境变量文件 (API Keys)
├── .env.example                              # 环境变量模版
├── .gitignore                                # Git 忽略配置
├── main_cli.py                               # CLI (命令行) 启动入口
├── pyproject.toml                            # 项目元数据与依赖配置
├── uv.lock                                   # [自动生成] 依赖版本锁定文件
└── README.md                                 # 项目说明文档
```
